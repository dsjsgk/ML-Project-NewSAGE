...
parser.add_argument('--attention', action='store_true', default=False, help='Using Attention Layer')

parser.add_argument('--random_walk', action='store_true', default=False, help='Using Random Walk Schemes')

parser.add_argument('--dataset',type = int, default=0, help='DataSet')


dataset = "cora"
if parser.parse_args().dataset==1 :
    dataset = "citeseer"
...
Data = namedtuple('Data', ['x', 'y', 'adjacency_dict', 'train_mask', 'val_mask', 'test_mask','degree'])
...
model = GraphSage(input_dim=INPUT_DIM, hidden_dim=HIDDEN_DIM, num_neighbors_list=NUM_NEIGHBORS_LIST,
                  ...
                  aggr_hidden_method="sum",attention=parser.parse_args().attention).to(DEVICE)
print(model)
...
def train():
    ...
    for e in range(EPOCHS):
        ...
        for batch in range(NUM_BATCH_PER_EPOCH):  # 每个epoch循环的批次数
            ...
            batch_sampling_result = multihop_sampling(batch_src_index, NUM_NEIGHBORS_LIST, data.adjacency_dict,data.degree,data.x)
...
def validate():
    ...
    with torch.no_grad():  # 关闭梯度
        val_sampling_result = multihop_sampling(val_index, NUM_NEIGHBORS_LIST, data.adjacency_dict,data.degree,data.x)
...
def test():
    model.eval()  # 测试模式
    with torch.no_grad():  # 关闭梯度
        test_sampling_result = multihop_sampling(test_index, NUM_NEIGHBORS_LIST, data.adjacency_dict,data.degree,data.x)
