...
class CoraData(object):
    ...
    def process_data(self):
        ...
        degree = np.zeros(len(adjacency_dict))
        # print(degree.shape[0])
        for i in range(degree.shape[0]) :
            degree[i] = len(adjacency_dict[i])
            # print(degree[i])
        # print()
        # 打印数据的信息
        ...
        return Data(x=x, y=y, adjacency_dict=adjacency_dict,
                    train_mask=train_mask, val_mask=val_mask, test_mask=test_mask,degree=degree)
...
class CiteseerData(object):
    ...
    def process_data(self):
        ...
        # 返回命名元祖
        return Data(x=x, y=y, adjacency_dict=adjacency_dict,
                    train_mask=train_mask, val_mask=val_mask, test_mask=test_mask,degree=None)
