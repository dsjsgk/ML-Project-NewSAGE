...
class NeighborAggregator(nn.Module):  # 继承了nn.Module，具有可学习的参数
    def __init__(self, input_dim, output_dim, use_bias=False, aggr_method="max"):
        """聚合节点邻居
...
class GATLayer(nn.Module) :
    ...
    def forward(self,src_node_features, neighbor_node_features):
        Wh1 = torch.matmul(src_node_features,self.W)
        Wh2 = torch.matmul(neighbor_node_features,self.W)
        _Val1 = torch.matmul(Wh1,self.a[:self.output_dim, :])
        _Val2 = torch.matmul(Wh2, self.a[self.output_dim:, :])
        nwVal = torch.squeeze(_Val2)
        e = self.leakyrelu(_Val1+nwVal)

        attention = F.softmax(e,dim = 1) 
        
        attention = F.dropout(attention,p=0.2,training = self.training)
        #changable parameter p 
        
        tmp = Wh2.permute(2,0,1)
        res = torch.mul(tmp,attention)
        tmp = res.permute(1,2,0)
        ans = tmp.sum(dim = 1)
        # print(ans.shape)
        return ans


#在aggregate的过程中加入attention层 IDEA3:trying
class SageGCN(nn.Module):
    def __init__(self, input_dim, hidden_dim, activation=nn.LeakyReLU(0.5), aggr_neighbor_method="mean", aggr_hidden_method="sum",heads_=3,merge_method = "mean",attention = False):
        """SageGCN层定义
        ...
        if self.attention :
            for i in range(heads_):
                self.aggregator.append(GATLayer(input_dim,hidden_dim))
                print(1)
        else :
        ...
        # 定义聚合器aggregator 此处进行修改，使用自己实现的Attention进行修改
        # self.aggregator = NeighborAggregator(input_dim, hidden_dim, aggr_method=aggr_neighbor_method)
        # self.aggregator = GATLayer(input_dim,hidden_dim)
        # 初始化更新的权重参数（无bias）
    ...
    def forward(self, src_node_features, neighbor_node_features):
        ...
        if (self.attention!=True) :
            ...
            ans = torch.zeros(src_node_features.shape[0],self.hidden_dim)
            # print(src_node_features.shape)
            # print(self.heads)
            for i in self.aggregator:
                ans = ans+i.forward(src_node_features,neighbor_node_features)
                # print("fuck")
            ...
            if(self.merge_method=='cat'):
                ...
                ans=ans/self.heads
            if self.activation:
                ans =self.activation(ans)
            else: 
                ans=ans
            ans = torch.nn.functional.normalize(ans, p=1, dim=1)
            return ans
...
class GraphSage(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_neighbors_list, aggr_neighbor_method="mean",
                 aggr_hidden_method="sum",attention = False):
        """
        GraphSAGE模型的定义
        ...
        self.gcn.append(SageGCN(input_dim, hidden_dim[0], aggr_neighbor_method=aggr_neighbor_method,
                                aggr_hidden_method=aggr_hidden_method,attention=self.attention))
        ...
        self.gcn.append(
            SageGCN(hidden_dim[-2], hidden_dim[-1], activation=None, aggr_neighbor_method=aggr_neighbor_method,
                    aggr_hidden_method=aggr_hidden_method,attention=self.attention))  # 最后一层不需要激活
